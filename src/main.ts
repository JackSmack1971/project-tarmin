...
<div class="hud" data-hud hidden><div><span class="eyebrow">THE UNDERCRYPT</span><strong data-floor></strong></div><div class="hud-stats"><span data-health></span><span>LEFT · EMPTY</span><span>RIGHT · EMPTY</span><span data-ring>RING · EMPTY</span></div><button type="button" data-pause>PAUSE</button><p class="feedback" data-feedback aria-live="polite"></p></div>
...
window.addEventListener("tarmin-state", (event) => {
  const detail = (event as CustomEvent<{ floor: number; turn: number; health: number; feedback: string; seed: number }>).detail;
