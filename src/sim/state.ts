...
export type GameEvent = { readonly type: "turned"; readonly facing: Facing } | { readonly type: "moved"; readonly position: Point } | { readonly type: "movementBlocked"; readonly position: Point } | { readonly type: "encounterStarted"; readonly name: string; readonly monsterId: string } | { readonly type: "attackAttempt"; readonly hand: HandSlot } | { readonly type: "hit"; readonly target: string; readonly damage: number } | { readonly type: "monsterAttack"; readonly monsterId: string; readonly damage: number } | { readonly type: "monsterDefeated"; readonly monsterId: string } | { readonly type: "playerDefeated" } | { readonly type: "itemAcquired"; readonly itemId: string } | { readonly type: "itemUsed"; readonly itemId: string } | { readonly type: "equipmentChanged"; readonly hand: HandSlot; readonly itemId: string | null } | { readonly type: "inventoryRotated"; readonly selectedItemId: string | null } | { readonly type: "itemDropped"; readonly itemId: string } | { readonly type: "commandIgnored"; readonly reason: "encounter" | "invalidAction" | "inventoryFull" | "emptyRing" | "noLoot" };
...
export function createInitialState(seed = 7391): GameState {
  ...
  return { rulesVersion: 2, seed, rngState: seed >>> 0 || 1, floor: 1, turn: 0, player: { position: { x: 1, y: 1 }, facing: "east" }, walls, doors: [], playerHealth: 10, playerMaxHealth: 10, monsters: [{ id: "monster-warden-1", definitionId: "ashbound-warden", position: { x: 2, y: 1 }, health: 5, defeated: false }], encounter: null, items: [item("item-ember-pike-1", "ember-pike", "ring"), item("item-moss-tonic-1", "moss-tonic", "ring"), item("item-star-seal-1", "star-forged-seal", "world", { x: 1, y: 1 })], leftHand: "item-ember-pike-1", rightHand: null, ring: ["item-ember-pike-1", "item-moss-tonic-1"], selectedRingIndex: 0, loot: [] };
}
...
function combat(state: GameState, hand: HandSlot, events: GameEvent[]): GameState {
  ...
  if (!playerHealth) events.push({ type: "playerDefeated" });
...
export function executeCommand(state: GameState, command: Command): CommandResult {
  ...
  if (command === "pickup") {
    ...
    return { state: advance({ ...state, loot: state.loot.filter((id) => id !== itemId), ring: [...state.ring, itemId], items: state.items.map((value): ItemInstance => value.id === itemId ? { ...value, location: "ring", position: undefined } : value) }, {}), events: [{ type: "itemAcquired", itemId }] };
  }
  ...
  return { state: advance(state, { player: { position, facing } }), events };
}
